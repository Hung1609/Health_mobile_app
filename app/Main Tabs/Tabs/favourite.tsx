...
const Favourite = () => {
  ...
  const filteredFavorites = selectedFavorite === 'All'
    ? favorites
    : favorites.filter((item: { ftype: string }) => item.ftype.toLowerCase() === selectedFavorite.toLowerCase());
  ...
  const checkFavorite = (item: FavoriteItem) => {
    if (item.ftype.toLowerCase() === 'nutrition') {
      router.push({
          pathname: '/Main Tabs/Home/nutrition-details', 
          params: { item: JSON.stringify(item) } 
      });
    } else if (item.ftype.toLowerCase() === 'workout') {
      router.push({
              ...
              <View className="w-full h-full flex-1 px-4">
                  <View className="flex-row items-center justify-between mt-10">
                      ...
                      <View className="flex-row space-x-1">
                          ...
                          <TouchableOpacity
                              onPress={() =>
                                  router.push("/Other Tabs/notification")
                              }
                              activeOpacity={0.7}
                          >
                              <Icon
                  ...
                  <View className="mt-10 flex-row items-center gap-3 justify-center">
                      {["All", "Workout", "Nutrition"].map((item) => (
                          <TouchableOpacity
                              key={item}
                              onPress={() =>
                                  setSelectedFavorite(item as FavouriteItems)
                              }
                              activeOpacity={0.7}
                              className={`justify-center items-center p-2 rounded-3xl w-28 border ${
