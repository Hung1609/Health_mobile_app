...
const Nutrition = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const response = await fetch(`${IP}:${PORT}/nutrition/${selectedMeal}`);
        const data = await response.json();

        const recommended = data.filter(
          (item: NutritionItem) => item.type === "r"
        );
        const recipes = data.filter((item: NutritionItem) => item.type !== "r");
                ...
                <View className="gap-3 flex-row mx-4">
                  {recommendedData.map((item) => (
                    <TouchableOpacity
                      ...
                      className="bg-gray-100 rounded-3xl overflow-hidden w-48 border-2 border-blue-500"
                    >
