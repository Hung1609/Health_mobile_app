...
const Progress = () => {
  const { workoutData, addWorkout } = useWorkout();
  const { userData } = useUser();
  const [selectedDate, setSelectedDate] = useState<string>(getToday());
  const [currentView, setCurrentView] = useState<"calendar" | "tracking">(
    "calendar"
  );

  // Fetch user data, including workoutContext, when the component mounts
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        if (response.ok) {
          // Update the WorkoutContext with the fetched workoutContext data
          const fetchedWorkouts = data.progress || [];

          if (fetchedWorkouts.length === 0) {
            console.log("No workouts found for this user.");
          }

          fetchedWorkouts.forEach((workout: any) => {
            addWorkout(workout, workout.date);
          });
        } else {
  ...
  // Save workoutData to the backend whenever it changes
  useEffect(() => {
    const saveWorkoutData = async () => {
      try {
        // Convert the id field to a string for each workout
        const formattedWorkoutData = workoutData.map((workout) => ({
          ...workout,
          id: String(workout.id), // Convert id to string
        }));

        const response = await fetch(`${IP}:${PORT}/save_workout_context`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
          body: JSON.stringify({
            ...
            progress: formattedWorkoutData,
          }),
        });

        const data = await response.json();
        if (response.ok) {
          console.log("Workout context saved successfully:", data.message);
        } else {
          console.error("Failed to save workout context:", data.detail);
        }
      } catch (error) {
        console.error("Error saving workout context:", error);
      }
    };
  ...
  const monthlyData = workoutData.reduce<
    Record<number, { calories: number; label: string }>
  >((acc, workout) => {
  ...
  const groupedData = workoutData.reduce<Record<string, GroupedWorkoutData>>(
    (acc, workout) => {
      if (!acc[workout.date]) {
        acc[workout.date] = {
          calories: 0,
          duration: [],
          dayName: getDayName(workout.date),
        };
      }
          ...
          <View className="mx-4">
            ...
            {currentView === "calendar" ? ( // Calendar
              <View>
                ...
                <View className="mb-2">
                  <Text className="mt-3 mb-2 text-xl font-bold text-yellow-500">
                    Activities
                  </Text>
                  {filteredActivities.length > 0 ? (
                    ...
                    <Text className="mx-2 mb-2 text-gray-500 text-center">
              ...
              <View className="flex-1 justify-center items-center mb-4">
                <Text className="text-yellow-500 text-xl font-bold mt-2 mb-8">
                  My progress in {currentYear}
                </Text>
                  ...
                  <BarChart
                    ...
                    chartConfig={{
                      ...
                      labelColor: (opacity = 1) =>
                        `rgba(234, 179, 80, ${opacity})`, // Label color
                      style: {
                        borderRadius: 16,
                      },
